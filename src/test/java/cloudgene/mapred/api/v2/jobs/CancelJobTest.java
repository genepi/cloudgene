...
public class CancelJobTest {
	...
	public void testCancelSleepJob() throws Exception {
		...
		Response response = RestAssured.given().header(accessToken).and().multiPart("input", "dummy").when()
				.post("/api/v2/jobs/submit/long-sleep").thenReturn();
		...
		RestAssured.given().header(accessToken).when().get("/api/v2/jobs/" + id + "/cancel").then().statusCode(200);
	...
	@Test
	public void testCancelWithWrongJobId() throws Exception {

		String id = "some-random-id";

		Header accessToken = client.loginAsPublicUser();

		RestAssured.given().header(accessToken).when().get("/api/v2/jobs/" + id + "/cancel").then().statusCode(404)
				.and().body("success", equalTo(false)).and().body("message", equalTo("Job " + id + " not found."));
	...
	public void testCancelWithoutLogin() throws Exception {
		...
		String id = "some-random-id";
		RestAssured.when().get("/api/v2/jobs/" + id + "/cancel").then().statusCode(401);
